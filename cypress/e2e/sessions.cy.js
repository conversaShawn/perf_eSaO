...
describe("Tests", () => {
  ...
  const login = (name) => {
    cy.session(name, () => {
        ...
        cy.get("button").click();
        cy.get("[data-test-id=dashboard-templates-teaser-grid]")
          .children()
...
//   });
  ...
  it.only("test 1", () => {
    ...
    // cy.get('[data-test-id=quick-insert-menu-list]').should('not.be.visible')
    cy.get('body').type(`{${modKey}+k}`)
  cy.get('[data-test-id=quick-insert-menu-list]').expect('to.be.visible')
    ...
    // cy.getByTestId("quick-insert-menu-list")
    //   .contains(accountSettingsLabel)
    //   .click();
    // cy.get(".account-settings .modal-body").should("be.visible");
  });
  ...
  it("test 3", () => {
    ...
    cy.get(".current-name .current-value")
      .should("be.visible")
      .and("have.text", newName);
  ...
  it("test 4", () => {
    const subscribedAllLabel = 'Subscribed to all' || 'Mentions only';
    ...
    cy.get(".account-property-row input").each((checkbox) =>
